...
create_terraform_backend_for_packer_infra() {
  log_message "Create Terraform Backend For Packer Infra"

  local output
  output=$(aws s3 mb "s3://${PACKER_INFRA_BUCKET_NAME}" --profile "${AWS_PROFILE}" 2>&1)
  local exit_code=$?

  if [ ${exit_code} -eq 0 ]; then
    log_message "Bucket ${PACKER_INFRA_BUCKET_NAME} created successfully"
  elif echo "${output}" | grep -q "BucketAlreadyOwnedByYou"; then
    log_message "Bucket ${PACKER_INFRA_BUCKET_NAME} already exists"
  else
